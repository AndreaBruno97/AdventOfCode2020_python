...
def newRound(old_matrix, size_x, size_y, size_z):
    ...
    for cur_z in range(size_z):
        for cur_y in range(size_y):
            for cur_x in range(size_x):
                ...
                but not when x, y or z have values 0 or len-1
                ...
                submatrix = old_matrix[z_min:z_max + 1, y_min:y_max + 1, x_min:x_max + 1]
                # Remember if the current seat is occupied, in that case it should be removed from the counter
                is_current_seat_occupied = old_matrix[cur_z][cur_y][cur_x]
                ...
                if is_current_seat_occupied:
                    if (not total_occupied == 2) and (not total_occupied == 3):
                        newMatrix[cur_z][cur_y][cur_x] = 0
                else:
                    if total_occupied == 3:
                        newMatrix[cur_z][cur_y][cur_x] = 1
...
matrix = matrix.reshape(max_size_z, max_size_y, max_size_x)


newMatrix = np.array([])
for cur_round in range(rounds):
    newMatrix = newRound(matrix, max_size_x, max_size_y, max_size_z)
